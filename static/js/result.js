    ...
    +json[i][1]
      +'</div><div class="modal-footer"><button type="button" class="btn btn-primary" data-dismiss="modal">Close</button></div></div></div></div>').insertAfter("#bar");
	             ...
	             $("#details").prepend('<a class="outside" style="padding-top:20px;color:#010101;width:100%;text-align:center;display:block" href = "'
	                        //+window.location.href+'out.predicted_gene_scores">View source data</a>'); // Kai made the change on 20180508 to get rid of index.html file name
	                        +window.location.href.replace(/[^/]*$/, '')+'out.predicted_gene_scores">View source data</a>');
	             $("a.outside").attr("target","_blank");
               });
